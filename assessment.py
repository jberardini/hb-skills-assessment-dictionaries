...
def word_length_sorted(words):
    ...
    word_lengths_unsorted = {}
    word_lengths_sorted = []

    for word in words:
        word_length = len(word)
        if word_length not in word_lengths_unsorted:
            word_lengths_unsorted[word_length] = [word]
            words_by_length = word_lengths_unsorted[word_length]
        else: 
            word_lengths_unsorted[word_length] = words_by_length + [word]

    for length, words in sorted(word_lengths_unsorted.items()):
        sorted_words = sorted(words)
        word_length_pair = (length, sorted_words)
        word_lengths_sorted.append(word_length_pair)


    return word_lengths_sorted
...
def translate_to_pirate_talk(phrase):
    ...
    for word in words_list:
        translated_word = english_to_pirate.get(word, word)
        translated_words_list.append(translated_word)

    translated_phrase = ' '.join(translated_words_list)
